...
namespace WashinqV2.Pages.Views.Cashier
{
    public partial class CashierAddOrder : Form
    {
        ...
        {
            if (cbxService.SelectedItem == null || string.IsNullOrEmpty(cbxService.SelectedItem.ToString()))
            {
                selectedServiceId = 0;
        ...
        private void btnPay_Click(object sender, EventArgs e)
        {
            ...
            if (cbxService.SelectedItem == null || selectedServiceId == 0)
            {
                MessageBox.Show("Pilih jenis layanan terlebih dahulu.",
                    "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                cbxService.Focus(); // Focus ke combo box service
                return;
            }
...
(@userId, @customerId, @serviceId, @totalQty, @totalPrice, @paid, @payment, @notes, @submittedAt, @takenAt)";
                ...
                using (var cmd = new MySqlCommand(insertQuery, conn))
                {
                    ...
                    try
                    {
                        ...
                        var payForm = new CashierCustomerPay(customerId, totalPrice);
                        payForm.ShowDialog();
